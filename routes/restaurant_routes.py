...
from models.restaurant import RestaurantCreate, RestaurantListMenuItem, RestaurantOut, RestaurantListItem, RestaurantUpdate
...
# Search Restaurants using name or description (public)   
@router.get("/search", response_model=list[RestaurantListItem])
async def search_restaurants(
    q: str,
    current_user: CurrentUser = Depends(get_current_user)
):
    try:
        # For simplicity, we search by name or description containing the query string (case-insensitive)
        # In production, consider using a full-text search engine for better performance and relevance
        restaurants = await restaurant_service.search_restaurants(q)
        return restaurants
    except Exception:
        logger.exception("Error searching restaurants")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal Server Error")
    
# Public: get single restaurant using the id (only if approved and not disabled)
